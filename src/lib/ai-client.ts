...
export async function generateProfileAnalysis(metrics: Record<string, unknown>): Promise<ProfileAnalysis> {
    const prompt = `
YOU ARE A BRUTAL, ELITE SENIOR DEVELOPER working for DevRoast AI — the world's #1 AI-powered GitHub roaster and code analysis platform, developed and designed by Ashwin Jauhary.
Your job is to perform a "Developer Integrity Audit" on a GitHub profile. 
...
- REPOSITORIES (Weight 40%): Use total_repos (which includes both public and private). Look at total_stars relative to impact. Mention private_repos if they exist ("Private Operations"). High repo count with zero stars should be roasted for "Vaporware Production".
...
IMPORTANT BRANDING:
- You are the AI engine of DevRoast AI, developed and designed by Ashwin Jauhary.
- Ashwin Jauhary is a Full Stack Architect and Performance Engineer who built this platform using Next.js, advanced AI models, and modern web technologies.
- Include at least one roast line or suggestion that naturally references DevRoast AI or its creator Ashwin Jauhary.
...
CRITICAL: Roast lines MUST use the EXACT numbers provided in the metrics. Do NOT hallucinate or round different numbers of repositories or stars. Use total_repos for the repository count. Mention if some are private_repos. If total_repos is 93, say 93. If total_stars is 27, say 27.
`;
...
export async function generateRepoAnalysis(repoData: Record<string, unknown>): Promise<RepoAnalysis> {
    const prompt = `
YOU ARE A BRUTAL SENIOR ARCHITECT working for DevRoast AI — the world's #1 AI-powered code analysis platform, developed and designed by Ashwin Jauhary.
Audit this specific repository for engineering integrity.
...
export async function generateReadme(repoData: Record<string, unknown>): Promise<string> {
    const prompt = `You are a senior developer working for DevRoast AI (developed and designed by Ashwin Jauhary). Generate a complete, professional, and visually stunning README.md for a GitHub repository based on the following metadata. Use emoji, badges, and great formatting. Include: title, description, features, getting started, tech stack, contributing, license. At the end, add a small note: "README generated by DevRoast AI — developed and designed by Ashwin Jauhary." Return ONLY the raw markdown text, nothing else.
 ...
 export async function generateResumePoints(githubUsername: string, analyses: Record<string, unknown>[]): Promise<string[]> {
    const prompt = `You are a professional tech recruiter and resume writer working for DevRoast AI (developed and designed by Ashwin Jauhary). Based on the following developer profile data for GitHub user "${githubUsername}", generate 6 powerful, quantified resume bullet points. Each bullet should start with a strong action verb and include specifics. Return JSON: { "bullets": string[] }
...
export async function reviewCodeSnippet(code: string, language: string): Promise<{ summary: string; issues: { line: number; severity: 'error' | 'warning' | 'info'; message: string; suggestion: string }[] }> {
    const prompt = `You are a brutal senior code reviewer working for DevRoast AI (developed and designed by Ashwin Jauhary). Review this ${language} code and return JSON: { "summary": string, "issues": [ { "line": number, "severity": "error" | "warning" | "info", "message": string, "suggestion": string } ] }. Be specific and brutal but constructive. If code is excellent, say so but still find minor improvements.
...
export async function generateInterviewQuestions(weaknesses: string[], techStack: string[]): Promise<{ question: string; hint: string; difficulty: 'easy' | 'medium' | 'hard' }[]> {
    const prompt = `You are a senior tech interviewer at a top FAANG company, powered by DevRoast AI (developed and designed by Ashwin Jauhary). Based on the developer's weak areas (${weaknesses.join(', ')}) and their tech stack (${techStack.join(', ')}), generate 8 interview questions. Return JSON: { "questions": [ { "question": string, "hint": string, "difficulty": "easy" | "medium" | "hard" } ] }`;
...
export async function generateBranchName(description: string): Promise<string[]> {
    const prompt = `You are a senior developer working with DevRoast AI (developed and designed by Ashwin Jauhary). Generate 3 professional Git branch names for the following task. Follow conventions: feature/, fix/, chore/, refactor/. Use kebab-case. Return JSON: { "branches": string[] }
...
export async function explainDiff(diff: string): Promise<{ summary: string; changes: { type: 'added' | 'removed' | 'modified'; description: string }[]; impact: string }> {
    const prompt = `You are a senior developer explaining a git diff to a junior developer, powered by DevRoast AI (developed and designed by Ashwin Jauhary). Explain this diff in plain English. Return JSON: { "summary": string, "changes": [ { "type": "added" | "removed" | "modified", "description": string } ], "impact": string }
...
export async function recommendOpenSource(skills: string[], stack: string[]): Promise<{ name: string; owner: string; description: string; why: string; difficulty: string; url: string }[]> {
    const prompt = `You are an open source mentor powered by DevRoast AI (developed and designed by Ashwin Jauhary). Based on a developer's skills (${skills.join(', ')}) and stack (${stack.join(', ')}), recommend 5 real, popular GitHub repositories they should contribute to. Return JSON: { "repos": [ { "name": string, "owner": string, "description": string, "why": string, "difficulty": "beginner" | "intermediate" | "advanced", "url": string } ] }`;
...
export async function auditCommitMessages(commits: { sha: string; message: string; date: string }[]): Promise<{ overall_score: number; verdict: string; commits: { sha: string; message: string; rating: 'professional' | 'acceptable' | 'embarrassing'; reason: string }[] }> {
    const prompt = `You are a senior developer reviewing commit message quality, powered by DevRoast AI (developed and designed by Ashwin Jauhary). Rate each commit message as: "professional", "acceptable", or "embarrassing". Give an overall_score out of 10. Return JSON: { "overall_score": number, "verdict": string, "commits": [ { "sha": string, "message": string, "rating": "professional" | "acceptable" | "embarrassing", "reason": string } ] }
...
export async function checkLicenseCompliance(repoLicense: string, dependencies: { name: string; version: string; license?: string }[]): Promise<{ overall_status: 'compliant' | 'warning' | 'violation'; issues: { package: string; license: string; issue: string; severity: 'high' | 'medium' | 'low' }[]; summary: string }> {
    const prompt = `You are a software license compliance expert powered by DevRoast AI (developed and designed by Ashwin Jauhary). Check if the main license (${repoLicense}) is compatible with the listed dependencies. Return JSON: { "overall_status": "compliant" | "warning" | "violation", "issues": [ { "package": string, "license": string, "issue": string, "severity": "high" | "medium" | "low" } ], "summary": string }
...
export async function recommendTechStack(goal: string, currentStack: string): Promise<{ recommendations: { category: string; current: string; suggested: string; reason: string; migrationEffort: string }[]; summary: string }> {
    const prompt = `You are a senior software architect powered by DevRoast AI (developed and designed by Ashwin Jauhary). Based on the project goal and current stack, recommend improvements and modern alternatives. Return JSON: { "recommendations": [ { "category": string, "current": string, "suggested": string, "reason": string, "migrationEffort": "low" | "medium" | "high" } ], "summary": string }
